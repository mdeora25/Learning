...
def update_damage(old_damage):
  ...
  for damage in old_damage:
    if (damage == 'Damages not recorded'):
      ...
      new_damage.append(float(damage[0:len_dam]) * float(con.get(damage[len_dam])))
  return new_damage 
...
def make_dict(name, month, year, Max_Sustained_Wind, Areas_Affected, Damage, Death):
  ...
  for i in range(0,len(name)):
    temp = dict(Name = name[i] , Month = months[i], Year = years[i], Max_Sustained_Wind =  max_sustained_winds[i] , Areas_Affected = areas_affected[i], Damage = damages[i], Death = deaths[i])
    hold[name[i]] = temp
  return hold
